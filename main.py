...
def get_groq_answer(user_message: str) -> str:
    ...
    # ЖЕСТКИЙ СИСТЕМНЫЙ ПРОМПТ: запрет на общие знания, опора строго на документы
    system_prompt = (
        "Ты — официальный информационный бот приемной комиссии РГСУ. "
        "ТВОЯ ГЛАВНАЯ И ЕДИНСТВЕННАЯ ЗАДАЧА — отвечать абитуриентам, используя СТРОГО предоставленные ниже материалы (Частые вопросы и Правила приема РГСУ). "
        "КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать собственные общие знания, домысливать, придумывать или отвечать «от себя». Если в документах нет информации — отправляй в приемную комиссию. "
        "Отвечай сплошным, связным текстом (абзацами), без таблиц, списков с вертикальными чертами, хэштегов и заголовков. "
        "Пиши как обычный человек в мессенджере.\n\n"
        f"=== БАЗА ЧАСТЫХ ВОПРОСОВ (FAQ) ===\n{retrieved_faq}\n\n"
    ...
    response = groq_client.chat.completions.create(
        ...
        temperature=0.1,  # Минимальная температура, чтобы модель вообще не фантазировала
        max_tokens=1000
    )
